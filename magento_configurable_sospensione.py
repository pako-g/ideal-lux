...
Output: configurable_products_lampada_a_sospensione.json

Usa le stesse REGOLE di magento_simple_sospensione_v2.py per raggruppare
...
# Importa REGOLE e helpers dal simple_v2
sys.path.insert(0, str(Path(__file__).parent))
from magento_simple_sospensione import (
...
INPUT_JSON         = "./file/simple_products_lampada_a_sospensione.json"
OUTPUT_JSON        = "./file/configurable_products_lampada_a_sospensione.json"
DESC_CSV           = "./file/descrizioni_configurabili_lampada_a_sospensione.csv"
...
def build_url_key(nome: str, sku: str) -> str:
    return re.sub(r"[^a-z0-9]+", "-", nome.lower()).strip("-") + "-" + sku.lower()
...
def build_configurable(
    ...
    url_key = build_url_key(titolo, config_sku.lower())
...
def raggruppa(varianti: pd.DataFrame, semplici_map: dict) -> dict:
    """
    Raggruppa i prodotti usando le stesse REGOLE del simple_v2.

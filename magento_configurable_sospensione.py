...
# Il valore è la chiave di gruppo da usare in sostituzione di estrai_sottofamiglia.
# I prodotti non matchati da nessuna regola usano la logica standard.
REGOLE_RAGGRUPPAMENTO = {
    # AMADEUS — un unico configurabile (asse config_modello SP6/SP8)
    r"ideal lux amadeus sp\d+":
        lambda m: "ideal lux amadeus lampada a sospensione",

    # ARIZONA — un unico configurabile (dimensioni diverse), nome → Arizona Pl
    r"ideal lux arizona sp\d+":
        lambda m: "ideal lux arizona pl lampada a sospensione",

    # AUDI-80 — un unico configurabile (SP5 e SP8 insieme, asse config_dimensioni)
    r"ideal lux audi-80 sp\d+":
        lambda m: "ideal lux audi-80 lampada a sospensione",

    # BRIGITTA — separa per numero luci (SP8 / SP12)
    r"ideal lux brigitta sp(\d+)":
        lambda m: f"ideal lux brigitta sp{m.group(1)} lampada a sospensione",

    # COLOSSAL — un unico configurabile (solo colore)
    r"ideal lux colossal sp\d+":
        lambda m: "ideal lux colossal lampada a sospensione",

    # CHALET — un unico configurabile (asse config_modello SP6/SP8/SP12)
    r"ideal lux chalet sp\d+":
        lambda m: "ideal lux chalet lampada a sospensione",

    # EQUINOXE SP1 — un unico configurabile (ambra, fumé, cromo, ottone + dimensioni)
    r"ideal lux equinoxe sp1\b":
        lambda m: "ideal lux equinoxe sp1 lampada a sospensione",

    # EQUINOXE SP3/SP6/SP8/SP12 — configurabili separati per numero luci
    r"ideal lux equinoxe sp(\d+)":
        lambda m: f"ideal lux equinoxe sp{m.group(1)} lampada a sospensione",

    # GALAXY — un unico configurabile (asse color + config_modello SP3/SP6)
    r"ideal lux galaxy sp\d+":
        lambda m: "ideal lux galaxy lampada a sospensione",

    # KALIQUE — separa per numero (-1/-2/-3), asse config_dimensioni
    r"ideal lux kalique-(\d+) sp\d+":
        lambda m: f"ideal lux kalique-{m.group(1)} lampada a sospensione",

    # LUMIERE — un unico configurabile (asse config_modello -1/-2/-3)
    r"ideal lux lumiere-\d+":
        lambda m: "ideal lux lumiere lampada a sospensione",

    # MINT — un unico configurabile (asse config_dimensioni, varianti -1/-2/-3)
    r"ideal lux mint-\d+ sp\d+":
        lambda m: "ideal lux mint lampada a sospensione",

    # UMILE SP1 e SP3 — separati, asse config_dimensioni (-1/-2/-3)
    r"ideal lux umile-\d+ sp(1)\b":
        lambda m: "ideal lux umile sp1 lampada a sospensione",
    r"ideal lux umile-\d+ sp(3)\b":
        lambda m: "ideal lux umile sp3 lampada a sospensione",
    r"ideal lux umile sp(1)\b":
        lambda m: "ideal lux umile sp1 lampada a sospensione",
    r"ideal lux umile sp(3)\b":
        lambda m: "ideal lux umile sp3 lampada a sospensione",

    # WAVES — un unico configurabile (asse config_dimensioni, varianti -1/-2/-3)
    r"ideal lux waves-\d+":
        lambda m: "ideal lux waves lampada a sospensione",
}
...
def estrai_sottofamiglia(nome: str) -> str:
    """Ritorna la chiave di raggruppamento per il prodotto semplice.
    Applica prima le REGOLE_RAGGRUPPAMENTO speciali, poi la logica standard."""
    base = re.split(r'-[A-Z]', nome)[0].strip()
    base_lower = base.lower()

    # Applica regole speciali nell'ordine in cui sono definite
    for pattern, key_fn in REGOLE_RAGGRUPPAMENTO.items():
        m = re.search(pattern, base_lower)
        if m:
            return key_fn(m)
...
def raggruppa_per_sottofamiglia(semplici: list) -> dict:
    ...
    for s in semplici:
        nome = s["product"]["name"]
        # Controlla se la famiglia è da saltare
        nome_lower = nome.lower()
        famiglia = nome_lower.replace("ideal lux ", "").split(" ")[0].split("-")[0]
        if famiglia in FAMIGLIE_DA_SALTARE:
            saltati += 1
            continue
        key = estrai_sottofamiglia(nome)
        gruppi.setdefault(key, []).append(s)
    if saltati:
        print(f"⏭️   Saltati {saltati} prodotti (famiglie escluse: {FAMIGLIE_DA_SALTARE})")
    return gruppi
...
def build_configurable(
    ...
    return {
        ...
        "_attr_codes": [] if is_single else attr_codes,
    }
